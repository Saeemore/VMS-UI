...
// Final datetime merge
const getISODateTime = () => {
  if (!newDate || !newTime) return null;
  return new Date(`${newDate}T${newTime}`).toISOString();
};

// Handle modal submit
const handleReschedule = async () => {
  const scheduled_at = getISODateTime();
  if (!scheduled_at) {
  ...
  try {
    await api.patch(`/host/visits/${selectedVisit._id}/reschedule`, {
      scheduled_at
    });
...
{showModal && (
  <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
    <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
      <h2 className="text-xl font-semibold mb-4">Reschedule Visit</h2>
...
<button
  className="mv-cta"
  type="button"
  disabled={it._isApproving}
  onClick={async () => {
    try {
      // Mark as approving
      setVisits((prev) =>
        prev.map((v) =>
          v._id === it._id ? { ...v, _isApproving: true } : v
        )
      );

      await api.patch(`/host/visits/${it._id}/approve`);

      // Remove approved visit from list
      setVisits((prev) => prev.filter((v) => v._id !== it._id));
      ...
      setVisits((prev) =>
        prev.map((v) =>
          v._id === it._id ? { ...v, _isApproving: false } : v
        )
      );
    }
  }}
>
  ...
  {it._isApproving ? "Approving..." : "Approve"}
</button>
              </li>
            );
          })}
